...
PGADMIN_COMPOSE=./tools/pgadmin4/docker-compose.pgadmin4.yml
SQITCH_COMPOSE=./tools/sqitch/docker-compose.sqitch.yml
...
define SERVERS_JSON
{
	"Servers": {
		"1": {
			"Name": "$(POSTGRES_HOST)/$(POSTGRES_DB)",
			"Group": "Servers",
			"Host": "$(POSTGRES_HOST)",
			"Port": 5432,
...
clear:
	...
	rm -f ./tools/pgadmin4/servers.json
sh:
	docker-compose -f $(MAIN_COMPOSE) -f $(SQITCH_COMPOSE) run --rm $(SQITCH_CONTAINER_NAME)
